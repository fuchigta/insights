...
| `update.check` | 新しいリリースが出ていないかの確認。`false` にすると更新のためにネットワークへ出ません | `true` |
| `update.interval` | 確認の間隔。前回からこれだけ経つまで問い合わせません | `24h0m0s` |
...
## 更新の確認と `insights update`

`insights` は自分自身のバージョンを確認し、新しいリリースが出ていれば標準エラーに知らせます。
**バイナリを勝手に置き換えることはしません。** 入れ替えるのは `insights update` を叩いたときだけです。

```bash
insights update --check   # 確認だけ
insights update           # 確認して、同意すれば置き換える
insights update --yes     # 確認プロンプトを省略する
```

通知が出るのは**標準エラーが端末のとき**だけです。cron やタスクスケジューラからの実行では
通知もネットワークへの問い合わせも行いません（定期実行のログを汚さないため）。
**定期実行しかしていない場合は、`insights config doctor` のバージョン欄で確認してください。**
現在の版・最新の版・実行ファイルを置き換えられるかがまとまって出ます。

確認結果は設定ファイルと同じディレクトリの `update-check.json` に記録され、`update.interval` の
あいだは再度問い合わせません。

### 更新のときに確かめていること

ダウンロードしたバイナリは、置き換える前に 2 段階で確かめます。

1. リリースに添付されている `.sha256` との照合（転送途中の破損・部分ダウンロードを弾く）
2. ダウンロードしたバイナリを実際に `--version` で起動し、期待するバージョンを名乗るかの確認
   （チェックサムは「壊れていないか」しか見ないため、アーキテクチャ違いのアセットを
   取り違えた場合はここで初めて分かります）

どちらかに失敗した場合、現在のバイナリには一切触れません。

### 更新できない場合

| 状況 | 挙動 |
|---|---|
| 実行ファイルに書き込む権限が無い（`/usr/local/bin` が root 所有など） | ダウンロードを始める前に止まります。`sudo insights update` で実行するか、README のインストール手順で入れ直してください |
| `go install` で導入した | 置き換えず、`go install github.com/fuchigta/insights/cmd/insights@latest` を案内します（GOBIN の管理下にあるバイナリを横から差し替えると、go が把握しているモジュールバージョンと実体がずれるため） |
| ソースからビルドした開発版 | 比較の基準になるバージョンが無いため、確認も更新も行いません |

Windows では実行中のファイルを削除できないため、置き換えの過程で `insights.exe.old` が
一時的に残ります。**次に `insights` を実行したときに自動で片付く**ので、手で消す必要はありません。
